...
class Maze {
	constructor(rows, columns) {
		...
		this.svg.setAttribute(
			"viewBox",
			`0 0 ${this.columns * this.cellSize} ${this.rows * this.cellSize}`
		);
	...
	calculateCellSize() {
		...
		const cellSize = Math.ceil(
			Math.min(maxWidth / this.columns, maxHeight / this.rows)
		);
    ...
    setup() {
        ...
        for (let r = 0; r < this.rows; r++) {
            ...
            for (let c = 0; c < this.columns; c++) {
                let cell = new Cell(
                    ...
                    this.wallGroup,
                    this.barcodeGroup,
                    this.startRow, // Pass startRow to the Cell constructor
                    this.startCol  // Pass startCol to the Cell constructor
                );
                row.push(cell);
        ...
        // Place the finish SVG on the starting tile
        current.addFinishSVG();
    }
	...
	draw() {
		...
		if (previous) {
			...
			if (
				(previous.getSpriteId() === "horizontal" ||
					previous.getSpriteId() === "vertical") &&
				this.barcodeCount < 5 &&
				Math.random() < 0.1
			) {
				previous.addBarcode();
	...
	saveMaze() {
        const mazeData = {
            ...
            grid: this.grid.map((row) =>
                row.map((cell) => ({
                    walls: cell.walls,
                    barcode: cell.hasBarcode ? cell.barcodeValue : null, // Include barcode value
                    starting_cell: cell.rowNum === this.startRow && cell.colNum === this.startCol
                }))
            ),
        };
        localStorage.setItem("savedMaze", JSON.stringify(mazeData)); // Save to localStorage
	...
	loadMaze() {
        ...
        if (savedMaze) {
            ...
            this.startRow = mazeData.startRow;
            this.startCol = mazeData.startCol;
            ...
            this.grid = mazeData.grid.map((row, rIdx) =>
                row.map((cellData, cIdx) => {
                    ...
                    cell.hasBarcode = cellData.barcode !== null;
                    cell.barcodeValue = cellData.barcode;
                    return cell;
                })
            );
	...
	drawLoadedMaze() {
        this.grid.forEach((row) =>
            row.forEach((cell) => {
                ...
                if (cell.hasBarcode) {
                    cell.addBarcode(cell.barcodeValue); // Add barcode with the stored value
                }
                // Ensure the starting tile retains its unique color and finish SVG
                if (cell.rowNum === this.startRow && cell.colNum === this.startCol) {
                    cell.markAsStartingTile();
                    cell.addFinishSVG(); // Add the finish SVG to the starting tile
                }
            })
        );
    }
...
class Cell {
	constructor(
        ...
        wallGroup,
        barcodeGroup,
        startRow, // Add startRow parameter
        startCol  // Add startCol parameter
    ) {
        this.rowNum = rowNum;
        this.colNum = colNum;
        ...
        this.startRow = startRow; // Store startRow
        this.startCol = startCol; // Store startCol
        this.barcodeValue = null; // Add a variable to store the barcode value
    }
	...
	show() {
        ...
        if (this.rowNum === this.startRow && this.colNum === this.startCol) {
	...
	clearTile(removeHighlight = true) {
		...
		let floorTiles = this.floorGroup.querySelectorAll(
			`rect[x="${x}"][y="${y}"]`
		);
		...
		// Clear the wall tile
		let wallTiles = this.wallGroup.querySelectorAll(
			`g[transform="translate(${x}, ${y})"]`
		);
		wallTiles.forEach((tile) => this.wallGroup.removeChild(tile));

		// Clear the barcode tile
		let barcodeTiles = this.barcodeGroup.querySelectorAll(
			`g[transform="translate(${x}, ${y})"]`
		);
	...
	getSpriteId() {
		...
		else if (!walls.top && !walls.right && walls.bottom && !walls.left)
			id = "T-from-top";
		else if (!walls.top && !walls.right && !walls.bottom && walls.left)
			id = "T-from-right";
		else if (walls.top && !walls.right && !walls.bottom && !walls.left)
	...
	highlight() {
		...
		let tiles = this.wallGroup.querySelectorAll(
			`g[transform="translate(${x}, ${y})"] use`
		);
	...
	addBarcode(barcodeValue) {
        if (this.hasBarcode && !barcodeValue) return; // Prevent adding a barcode if one already exists
        ...
        // Set the barcode value if provided
        if (barcodeValue) {
            this.barcodeValue = barcodeValue;
        } else {
            this.barcodeValue = Math.random()
                .toString(2)
                .substring(2, 10)
                .padStart(8, "0"); // Generate a random 8-bit binary string
        }

        // Generate the barcode SVG
        let barcode = generateBarcodeSVG(this.barcodeValue, this.cellSize);
        ...
        barcode.addEventListener("click", (event) => {
            ...
            showBarcodePopup(this.barcodeValue, event.clientX, event.clientY);
        });
    ...
    addFinishSVG() {
        ...
        // Create group for the finish SVG
        let group = document.createElementNS(svgNS, "g");
        group.setAttribute("transform", `translate(${x}, ${y})`);
    
        // Create the finish SVG element
        let finishSVG = document.createElementNS(svgNS, "image");
        finishSVG.setAttributeNS("http://www.w3.org/1999/xlink", "href", "images/finish-01.svg");
        finishSVG.setAttribute("width", this.cellSize-(this.cellSize*0.32));
        finishSVG.setAttribute("height", this.cellSize-(this.cellSize*0.32));
        finishSVG.setAttribute("transform", `translate(-${((this.cellSize)/2)-(this.cellSize*0.16)}, -${((this.cellSize)/2)-(this.cellSize*0.16)})`);
        finishSVG.setAttribute("class", "finish-tile");
    
        group.appendChild(finishSVG);
        this.barcodeGroup.appendChild(group); // Append to barcodeGroup
    
        requestAnimationFrame(() => {
            finishSVG.classList.add("visible");
        });
    }
	...
	markAsStartingTile() {
        ...
        // Add the finish SVG to the starting tile
        this.addFinishSVG();
    }
}
